...
export default (appConfig, preloadedState) => {
  ...
  const allActionCreators = R.map(
    actionCreator => (...args) => actionCreator(...args, config),
    _allActionCreators
  );
  ...
  const allReducers = R.map(
    reducer => (...args) => reducer(...args, config),
    _allReducers
  );
  ...
  const allMiddlewares = R.map(
    middleware => store => middleware(store, config),
    _allMiddlewares
  );
  ...
  setInterval(async () => {
    const hasLocalEvents = R.values(AppStore.getState().events).some(
      R.prop("localOnly")
    );
