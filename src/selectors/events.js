...
const interestingEventsSelector = createSelector(eventsSelector, events => {
  return R.values(events).filter(
    event =>
      event.relevantDocsIds !== undefined && event.relevantDocsIds.length > 0
  );
});

const sortedEventsSelector = createSelector(
  interestingEventsSelector,
  events => {
...
export const eventsByRelevantDocSelector = createSelector(
  sortedEventsSelector,
  events => {
    const eventsWithDocIds = R.flatten(
      events.map(event => {
    ...
    const eventsGroupedByDocId = R.groupBy(
      event => event.docId,
      eventsWithDocIds
    );
    return R.map(events => {
      return events.filter(event => {
        const appliedOnClientRev = R.path(
          ["appliedOnClient", event.docId],
          event
        );
